...
function brandsBox(b) {
  ...
  this.widget = jQuery("<div></div>", {"class":"brandsbox_container"}).append(jQuery("<div></div>", {"class":"brandsbox_inner"}));
  this.renderItems = function(b) {
    function c() {
      if ("undefined" !== typeof d) {
        jQuery(a.widget).find(".brandsbox_inner").html("");
        for (var c in d) {
          itemObject = d[c], qx = jQuery(itemObject).attr("href")?jQuery(itemObject).attr("href"):jQuery(itemObject).attr("url"), imageElement = qx ? jQuery('<a class="brandsbox_item_image"></a>').attr({href:function() {
            itemHref = qx;
            "undefined" == typeof itemHref && (itemHref = "javascript:void(0);");
            return itemHref;
          }, target:function() {
            itemTarget = jQuery(itemObject).attr("target");
            "undefined" == typeof itemTarget && (itemTarget = "_SELF");
            return itemTarget;
          }}) : jQuery('<span class="brandsbox_item_image"></span>'), jQuery(imageElement).css({"background-image":'url("' + jQuery(itemObject).attr("src") + '")', opacity:b.startOpacity}), jQuery(a.widget).find(".brandsbox_inner").append(jQuery('<div class="brandsbox_item"></div>').append(imageElement));
        }
      }
    }
    ...
    switch(typeof g) {
      ...
      case "string":
        d = [];
        jQuery.ajax({url:g, async:!1, success:function(a) {
          jQuery(a).find("item").each(function(a) {
            d[a] = {src:jQuery(this).attr("src"), url:jQuery(this).attr("url"), target:jQuery(this).attr("target")};
          });
          c();
        }, error:function() {
      ...
      f = setTimeout(function() {
        ...
        if ("single" == a.settings.scrollingMode) {
          if ("vertical" == a.settings.scrollingOrientation) {
            ...
            jQuery(a.widget).find(b).appendTo(jQuery(a.widget).find(".brandsbox_inner"));
          } else {
            for (c = 0;c < parseInt(a.settings.rows);c++) {
              itemToMoveIndex = 2 * a.settings.cols * c, appendAfterIndex = parseInt(itemToMoveIndex + 2 * a.settings.cols - 1), lastItemIndex = a.settings.collection.length - 1, appendAfterIndex > lastItemIndex && (appendAfterIndex = lastItemIndex), jQuery(a.widget).find(".brandsbox_item:eq(" + itemToMoveIndex + ")").insertAfter(jQuery(a.widget).find(".brandsbox_item:eq(" + appendAfterIndex + ")"));
            }
          }
        } else {
          if ("vertical" == a.settings.scrollingOrientation) {
            ...
            for (c = 0;c < parseInt(a.settings.rows);c++) {
              for (itemToMoveIndex = 2 * a.settings.cols * c, appendAfterIndex = parseInt(itemToMoveIndex + 2 * a.settings.cols - 1), lastItemIndex = a.settings.collection.length - 1, appendAfterIndex > lastItemIndex && (appendAfterIndex = lastItemIndex), b = itemToMoveIndex;b < parseInt(itemToMoveIndex + a.settings.cols);b++) {
                appendAfterIndex = parseInt(b + 2 * a.settings.cols - 1), jQuery(a.widget).find(".brandsbox_item").eq(b).insertAfter(jQuery(a.widget).find(".brandsbox_item").eq(appendAfterIndex));
              }
            }
          }
        }
        jQuery(a.widget).addClass("in");
        f = setTimeout(function() {
          jQuery(a.widget).removeClass("out").removeClass("in");
          k = !0;
          a.runTimer();
  ...
  this.destroy = function() {
    a.stop();
    jQuery(a.widget).remove();
  };
  this.settings = this.setOptions(b);
  jQuery(this.widget).on("mouseenter, mousemove", function() {
    a.pause();
  }).on("mouseleave", function() {
    a.play();
  }).on("mouseenter, mousemove", ".brandsbox_item", function() {
  }).on("mouseenter", ".brandsbox_item", function() {
    jQuery(this).find(".brandsbox_item_image").css({opacity:a.settings.hoverOpacity});
  }).on("mouseout, mouseleave", ".brandsbox_item", function() {
    jQuery(this).find(".brandsbox_item_image").css({opacity:a.settings.startOpacity});
  });
...
jQuery.extend({brandsBox:{embed:function(b, a) {
  "undefined" == typeof _brandsBox_instances[b] && (_brandsBox_instances[b] = new brandsBox(a), jQuery(b).append(_brandsBox_instances[b].widget), colsNum = _brandsBox_instances[b].settings.cols, rowsNum = _brandsBox_instances[b].settings.rows, cellWidth = parseInt(jQuery(b).innerWidth() / colsNum), cellHeight = parseInt(jQuery(b).innerHeight() / rowsNum), jQuery(_brandsBox_instances[b].widget).find(".brandsbox_item").css({width:cellWidth + "px", height:cellHeight + "px"}), setTimeout(function() {
    jQuery(_brandsBox_instances[b].widget).attr({"data-hover-effect-speed":_brandsBox_instances[b].settings.hoverEffectSpeed, "data-color-mode":_brandsBox_instances[b].settings.colorMode, "data-scrolling-mode":_brandsBox_instances[b].settings.scrollingMode, "data-scrolling-transition":_brandsBox_instances[b].settings.scrollingTransition, "data-scrolling-transition-speed":_brandsBox_instances[b].settings.scrollingTransitionSpeed}).addClass(_brandsBox_instances[b].settings.scrollingOrientation);
    setTimeout(function() {
      jQuery(_brandsBox_instances[b].widget).attr("data-ready", !0);
      setTimeout(function() {
